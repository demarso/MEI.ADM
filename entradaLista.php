...
<html lang="pt-br">
  ...
  <div class="container">
    ...
    <font class="text-warning" face="Verdana" size="7px">Lista Financeiro Faturamento</font>
...
<br />
  <h2 class="text-info">Entradas de <? echo date('d/m/Y',strtotime($data1))." a ".date('d/m/Y',strtotime($data2)) ?></h2>
   ...
   <thead>
    ...
    $dataH = date('Y/m/d');
		$con = 1; $soma = 0;
		$sql = "SELECT *, DATE_FORMAT(Datacad,'%d/%m/%Y') AS dat FROM tblEntrada WHERE Datacad >= '$data1' AND Datacad <='$data2' AND chave='$chave' ORDER BY Datacad DESC";
